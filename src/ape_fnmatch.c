...
#define MOON_FLAG_NAME "apr_fnmflags_t"
#define MOON_FLAG_TYPE int
#define MOON_FLAG_SUFFIX fnmflags
#include "moon_flag.h"
...
static int ape_fnmatch( lua_State* L ) {
  ...
  int flags = moon_flag_get_fnmflags( L, 3 );
...
APE_API void ape_fnmatch_setup( lua_State* L ) {
  /***
    ...
    @section fnmatch
  */
  luaL_Reg const ape_fnmatch_functions[] = {
  /***
    ...
    @treturn boolean true if the pattern matches the name, false
      otherwise
  */
    { "fnmatch", ape_fnmatch },
  /***
  ...
  */
  ...
  moon_register( L, ape_fnmatch_functions );
}
