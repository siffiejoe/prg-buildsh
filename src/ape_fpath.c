...
static int ape_filepath_merge( lua_State* L ) {
  ...
  apr_int32_t flags = moon_flag_get_fpflags( L, 3 );
...
static int ape_filepath_get( lua_State* L ) {
  apr_int32_t flags = moon_flag_get_fpflags( L, 1 );
  apr_pool_t** pool = ape_opt_pool( L, 2 );
  char* res = NULL;
...
APE_API void ape_fpath_setup( lua_State* L ) {
  /***
    ...
    @section filepath
  */
  luaL_Reg const ape_fpath_functions[] = {
  /***
    ...
    @tparam[opt] apr_pool_t pool a memory pool for temporary
      allocations
    @treturn string,string the root path and a file path relative to
      that
  ...
  */
  ...
  /***
    Splits an encoded list of file paths (like in the `PATH`
    environment variable) into its components.
  ...
  */
  ...
  /***
    Strips the directory part of a file path and returns the file
    name.
  ...
  */
  ...
  moon_flag_def_fpflags( L, 0 );
  /***
    @class field
    @name FILEPATH_NULL
  */
  moon_flag_new_fpflags( L, 0 );
  ...
  */
  ...
  moon_register( L, ape_fpath_functions );
}
