...
#define MOON_FLAG_NAME "apr_fileattrs_t"
#define MOON_FLAG_TYPE apr_fileattrs_t
#define MOON_FLAG_SUFFIX fattrs
#include "moon_flag.h"

#define MOON_FLAG_NAME "apr_oflags_t"
#define MOON_FLAG_TYPE apr_int32_t
#define MOON_FLAG_SUFFIX oflags
#include "moon_flag.h"

#define MOON_FLAG_NAME "apr_fileperms_t"
#define MOON_FLAG_TYPE apr_fileperms_t
#define MOON_FLAG_SUFFIX fperms
#define MOON_FLAG_USECACHE
#include "moon_flag.h"

#define MOON_FLAG_NAME "apr_flock_t"
#define MOON_FLAG_TYPE int
#define MOON_FLAG_SUFFIX flock
#include "moon_flag.h"
...
#endif
  push_if_flag( "prot", APR_FINFO_PROT, moon_flag_new_fperms( L, fi->protection ) );
...
static int ape_file_copy( lua_State* L ) {
  char const* fpath = luaL_checkstring( L, 1 );
  char const* tpath = luaL_checkstring( L, 2 );
  apr_fileperms_t perms = moon_flag_get_fperms( L, 3 );
...
static int ape_file_dup( lua_State* L ) {
  ...
  apr_file_t** newf = moon_newobject_ref( L, APE_FILE_NAME, 2 );
  apr_status_t rv = APR_EBADF;
  lua_pushvalue( L, 2 );
...
static int ape_file_namedpipe_create( lua_State* L ) {
  ...
  apr_fileperms_t perms = moon_flag_get_fperms( L, 2 );
  apr_pool_t** pool = ape_opt_pool( L, 3 );
...
static int ape_file_lock( lua_State* L ) {
  apr_file_t** file = moon_checkudata( L, 1, APE_FILE_NAME );
  int flag = moon_flag_get_flock( L, 2 );
  apr_status_t rv = APR_EBADF;
  if( *file != NULL )
...
static int ape_file_flags_get( lua_State* L ) {
  ...
  moon_flag_new_oflags( L, apr_file_flags_get( *file ) );
  return 1;
}
...
static int ape_file_attrs_set( lua_State* L ) {
  char const* path = luaL_checkstring( L, 1 );
  apr_fileattrs_t attr = moon_flag_get_fattrs( L, 2 );
  apr_fileattrs_t amask = moon_flag_get_fattrs( L, 3 );
  apr_pool_t** pool = ape_opt_pool( L, 4 );
...
static int ape_dir_open( lua_State* L ) {
  char const* name = luaL_checkstring( L, 1 );
  apr_pool_t** pool = moon_checkudata( L, 2, APE_POOL_NAME );
  apr_dir_t** dir = moon_newobject_ref( L, APE_DIR_NAME, 2 );
  lua_pushvalue( L, 2 );
  moon_setuvfield( L, -2, "pool" );
...
static int ape_file_open( lua_State* L ) {
  char const* name = luaL_checkstring( L, 1 );
  apr_int32_t flags = moon_flag_get_oflags( L, 2 );
  apr_fileperms_t perms = moon_flag_get_fperms( L, 3 );
  apr_pool_t** pool = moon_checkudata( L, 4, APE_POOL_NAME );
  apr_file_t** file = moon_newobject_ref( L, APE_FILE_NAME, 4 );
  lua_pushvalue( L, 4 );
  moon_setuvfield( L, -2, "pool" );
...
static int ape_file_open_stderr( lua_State* L ) {
  ...
  apr_file_t** file = moon_newobject_ref( L, APE_FILE_NAME, 1 );
  lua_pushvalue( L, 1 );
  moon_setuvfield( L, -2, "pool" );
...
static int ape_file_open_flags_stderr( lua_State* L ) {
  apr_int32_t flags = moon_flag_get_oflags( L, 1 );
  apr_pool_t** pool = moon_checkudata( L, 2, APE_POOL_NAME );
  apr_file_t** file = moon_newobject_ref( L, APE_FILE_NAME, 2 );
  lua_pushvalue( L, 2 );
  moon_setuvfield( L, -2, "pool" );
...
static int ape_file_pipe_create( lua_State* L ) {
  apr_pool_t** pool = moon_checkudata( L, 1, APE_POOL_NAME );
  apr_file_t** inf = moon_newobject_ref( L, APE_FILE_NAME, 1 );
  apr_file_t** outf = moon_newobject_ref( L, APE_FILE_NAME, 1 );
  lua_pushvalue( L, 1 );
  moon_setuvfield( L, -3, "pool" );
  lua_pushvalue( L, 1 );
  moon_setuvfield( L, -2, "pool" );
...
static int ape_file_pipe_create_ex( lua_State* L ) {
  ...
  apr_pool_t** pool = moon_checkudata( L, 2, APE_POOL_NAME );
  apr_file_t** inf = moon_newobject_ref( L, APE_FILE_NAME, 2 );
  apr_file_t** outf = moon_newobject_ref( L, APE_FILE_NAME, 2 );
  lua_pushvalue( L, 2 );
  moon_setuvfield( L, -3, "pool" );
  lua_pushvalue( L, 2 );
  moon_setuvfield( L, -2, "pool" );
...
static int ape_file_mktemp( lua_State* L ) {
  ...
  apr_file_t** file = moon_newobject_ref( L, APE_FILE_NAME, 3 );
...
#define define_flag( L, name, suffix ) \
  do { \
    moon_flag_new_ ## suffix( (L), APR_ ## name ); \
    lua_setfield( (L), -2, #name ); \
  } while( 0 )

APE_API void ape_file_setup( lua_State* L ) {
  luaL_Reg const ape_file_metamethods[] = {
    { "__gc", ape_file_gc },
    { NULL, NULL }
  };
  /***
  ...
  */
    { "sync", ape_file_sync },
  /***
    Makes sure that all written data is synced to disk (even stricter
    than `sync`).
    @function datasync
    @treturn boolean true on success
    @treturn nil,string,number nil, an error message, and an error
  ...
  */
  ...
  moon_object_type const ape_file_type = {
    APE_FILE_NAME,
    sizeof( apr_file_t* ),
    ape_file_init,
  ...
  luaL_Reg const ape_dir_metamethods[] = {
    { "__gc", ape_dir_gc },
    { NULL, NULL }
  };
  /***
    ...
    @type apr_dir_t
  */
  luaL_Reg const ape_dir_methods[] = {
  /***
  ...
  */
  ...
  moon_object_type const ape_dir_type = {
    APE_DIR_NAME,
    sizeof( apr_dir_t* ),
    ape_dir_init,
    ape_dir_metamethods,
  ...
  /***
    ...
    @section file
  */
  luaL_Reg const ape_file_functions[] = {
  /***
  ...
  */
  ...
  moon_defobject( L, &ape_file_type, 0 );
  moon_defobject( L, &ape_dir_type, 0 );
  moon_flag_def_fattrs( L, 0 );
  moon_flag_def_oflags( L, 0 );
  moon_flag_def_fperms( L, 0 );
  moon_flag_def_flock( L, 0 );
  /***
    @class field
  ...
  */
  ...
  moon_register( L, ape_file_functions );
}
...
APE_API void ape_file_userdata( lua_State* L, apr_file_t* f, int index ) {
  ...
  nf = moon_newobject_ref( L, APE_FILE_NAME, index );
  lua_insert( L, -2 );
  moon_setuvfield( L, -2, "pool" );
#if 1
  (void)pool;
